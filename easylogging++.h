...
/**
* @date : 2019-7-20 21:37:30 liyuan
* @brief 删除内存的宏
*/
...
/**
* @date : 2019/07/20 liyuan
* @brief 定义_ELPP_STRICT_ROLLOUT，这样在日志输出的过程中也会检查日志大小是否需要回滚；
*	否则只会在应用程序启动时才判断是否需要回滚；
*	这种回滚判断会造成一定的性能下降，但是对于客户端来说影响不大；
*	配置文件中配置ROLL_OUT_SIZE项，以字节为单位
*/
#define _ELPP_STRICT_ROLLOUT
/**
* @date : 2019/07/20 liyuan
* @brief 如果是多线程程序，定义下面的这些宏，并且这些宏要在包含头文件之前定义；
*/
...
/**
* @date : 2019/05/18 liyuan
* @brief 针对windows平台的函数名使用__FUNCTION__，这样函数名比较精简
* __FUNCSIG__/__FUNCDNAME__/__FUNCTION__，这三个宏定义区别参考：https://app.yinxiang.com/shard/s10/nl/24536/daa84b09-bf7c-4b56-9a52-8f407e3bf97e
* 简单来说，__FUNCSIG__是函数签名，__FUNCDNAME__是函数修饰名
*/
...
/**
* @date : 2019/05/19 liyuan
* @brief added
*/
#if _ELPP_OS_UNIX
#define		PATH_SLASH_MACRO                    '/'
#elif _ELPP_OS_WINDOWS
#define		PATH_SLASH_MACRO                    '\\'
#endif
...
//!
	...
	* @date 2019/05/18
	* @version ver1.0
	* @param pName:文件的全名，包括路径
	* @remark 1.2019/05/18 21:54 created by liyuan
	* @return 返回文件名，不包含路径名
	*/
	inline static const char* get_file_name(const char* pName)
	{
		if(pName == NULL)return NULL;
		const char *p = strrchr(pName, PATH_SLASH_MACRO);
		if(p == NULL)
			p = pName;
		else
			p += 1;
	...
	* @date 2019/05/19
	* @version ver1.0
	* @param pName:文件名，不包含路径，如果包含路径名，则返回值可能错误；
	* @remark 1.2019/05/19 21:08 created by liyuan
	* @return
	*/
	inline static const char* get_base_name(const char* pName){
		if(pName == NULL)return NULL;
		const char *p = strrchr(pName, '.');
		const char *p1 = strrchr(pName, PATH_SLASH_MACRO);
		if(p1 > p)return NULL;
		if(p == NULL)
			p = pName;
		else
...
public:
	/**
	* @date : 2019/05/19 liyuan
	* @brief add RollOutCount,default is 5
	*/
    enum {
        Enabled = 0, ToFile = 1, ToStandardOutput = 2, Format = 4, Filename = 8,
        MillisecondsWidth = 16, PerformanceTracking = 32, RollOutSize = 64, RollOutCount = 128, Unknown = 1010
    };

    static const unsigned int kMinValid = Enabled;
    //static const unsigned int kMaxValid = RollOutSize;
	static const unsigned int kMaxValid = RollOutCount;
    ...
    static std::string convertToString(unsigned int configurationType_) {
		...
		case RollOutCount:
			return std::string("ROLL_OUT_COUNT");
    ...
    static unsigned int convertFromString(const std::string& configStr) {
		...
		if (configStr == "roll_out_count" || configStr == "ROLL_OUT_COUNT") return ConfigurationType::RollOutCount;
...
public:
	...
	/**
	* @date : 2019/05/19 liyuan
	* @brief added for check file path is relative or absolute,we just check hard disk file, not url or other;
	*/
	static bool isAbsolutePath(const std::string& path_){
		if(path_.empty())return false;
#if _ELPP_OS_WINDOWS
		/**
		* @date : 2019/05/19 liyuan
		* @brief there is a windows api:PathIsRelative,but this api need link Shlwapi.lib and include <shlwapi.h>
		*	so we just check path second character is whether ':'
		*/
			return true;
#else
			return true;
...
#   endif // _ELPP_NDK
		...
		/**
		* @date : 2019/05/19 liyuan
		* @brief add RollOutCount
		* @tag
		*/
		setAll(easyloggingpp::ConfigurationType::RollOutCount, "0");
...
private:
	...
	/**
	* @date : 2019/05/19 liyuan
	* @brief add rolloutcount map
	*/
	internal::ConfigurationMap<unsigned char> rollOutCountMap_;
    internal::Constants* constants_;
	...
	* @brief added
	*/
	inline unsigned char rollOutCount(unsigned int level_){
		return rollOutCountMap_.get(level_);
	}
    ...
    void parse(const Configurations& configurations_) {
        for (std::size_t i = 0; i < configurations_.count(); ++i) {
            ...
            case ConfigurationType::RollOutSize:
				...
				/**
				* @date : 2019/05/19 liyuan
				* @brief do not rollout at config step
				*/
//                 unsigned int validLevel_ = 0;
//                 std::string rolloutFilename_ = std::string();
//                 checkRollOuts(conf->level(), validLevel_, rolloutFilename_);
                break;
			case ConfigurationType::RollOutCount:
				setValue(conf->level(), static_cast<unsigned char>(getULong(conf->value())), rollOutCountMap_);
            }
        }
    }
...
private:
    ...
    void buildAndWriteLine(void) {
        ...
        if (f_ & constants_->kLocation) {
			/**
			* @date : 2019/05/18 liyuan
			* @brief only write file name,not include file path
			*/
            //tempss_ << file_ << ":" << line_;
			tempss_ << get_file_name(file_) << ":" << line_;
    ...
    void safeWriteToFile(unsigned int level_, Logger* logger_, const std::string& line) {
		...
		/**
		* @date : 2019-8-6 11:44:46 liyuan
		* @brief 这里做个判断，如果文件没有权限写的话直接输出标准流，否则会崩溃；
		*/
        if(fstr != NULL){
			(*fstr) << line;
			fstr->flush();
			...
			std::cout << line;
        Logger* currLogger_ = NULL;
        for (std::size_t i = 0; i < registeredLoggers->count() && fstr; ++i) {
            currLogger_ = registeredLoggers->list().at(i);
...
/**
* @date : 2019/05/17 liyuan
* @brief added macro for more convenient
* @tag
*/
#define USING_EASYLOG
#ifdef USING_EASYLOG
...
#define log_qa				LOG(QA)
#define log_verbose			LOG(VERBOSE)
#define log_trace			LOG(TRACE)

#define logi_if(x)			LOG_IF(x, INFO)
#define logd_if(x)			LOG_IF(x, DEBUG)
#define logw_if(x)			LOG_IF(x, WARNING)
#define loge_if(x)			LOG_IF(x, ERROR)
#define logf_if(x)			LOG_IF(x, FATAL)
#define logq_if(x)			LOG_IF(x, QA)
#define logv_if(x)			LOG_IF(x, VERBOSE)
#define logt_if(x)			LOG_IF(x, TRACE)
...
#define log_qa			log_info
#define log_verbose		log_info
#define log_trace		log_info

#define logi_if(x)			if(x)log_info
#define logd_if(x)			if(x)log_info
#define logw_if(x)			if(x)log_info
#define loge_if(x)			if(x)log_info
#define logf_if(x)			if(x)log_info
#define logq_if(x)			if(x)log_info
#define logv_if(x)			if(x)log_info
#define logt_if(x)			if(x)log_info
...
#define	stack_trace					easyloggingpp::StackTrace stackLog(__FILE__, __func__, __LINE__)
class StackTrace
{
public:
	explicit StackTrace(const char *filename, const char *fun_name, const long& lineNum)
	{
		_ELPP_GET_CURR_TIME(&m_start);
		m_fileName = filename;
		m_funcName = fun_name;
		m_lineNum = lineNum;
		easyloggingpp::internal::Writer("trivial", easyloggingpp::internal::Aspect::Normal, easyloggingpp::Level::Trace, m_funcName.c_str(), m_fileName.c_str(), m_lineNum) << "Begin to executed [" << m_funcName << "]";
		easyloggingpp::internal::Writer("trivial", easyloggingpp::internal::Aspect::Normal, easyloggingpp::Level::Info, m_funcName.c_str(), m_fileName.c_str(), m_lineNum) << "Begin " << m_funcName;
	}

	~StackTrace()
	{
		_ELPP_GET_CURR_TIME(&m_end);
		std::string sDiff = easyloggingpp::internal::utilities::DateUtils::formatMilliSeconds(
		easyloggingpp::internal::utilities::DateUtils::getTimeDifference(m_end, m_start));
		easyloggingpp::internal::Writer("trivial", easyloggingpp::internal::Aspect::Normal, easyloggingpp::Level::Trace, m_funcName.c_str(), m_fileName.c_str(), m_lineNum) << "Executed [" << m_funcName << "] in [" << sDiff << "]";
		easyloggingpp::internal::Writer("trivial", easyloggingpp::internal::Aspect::Normal, easyloggingpp::Level::Info, m_funcName.c_str(), m_fileName.c_str(), m_lineNum) << "Finished " << m_funcName;
	}
